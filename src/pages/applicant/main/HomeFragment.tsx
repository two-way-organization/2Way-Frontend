...
import { Appbar, Card, Text, TextInput, TouchableRipple } from 'react-native-paper';
import { hFull, m8, mx16, my8, p16, p8, px16, px8, py16, wFull } from '../../../common/style';
import Icons from '@expo/vector-icons/MaterialIcons';
import { useQuery } from '@tanstack/react-query';
import { fetchRandomJobs, fetchRecentJobs } from '../../../api/application';
...
export const ApplicantHomeFragment = () => {
  const { data: randomData } = useQuery({
    ...
    queryFn: async () => await fetchRandomJobs(),
  });
  const { data: recentData } = useQuery({
    queryKey: ['applications/recent'],
    queryFn: async () => await fetchRecentJobs(),
  });
  ...
  return (
    <View style={[wFull, hFull]}>
      ...
      <View
        style={[p16, { backgroundColor: colors.primary, paddingTop: 0 }]}>
        <TextInput
          mode={'outlined'}
          theme={{
            colors: {
